 ...
 * Реагирует на уведомления от Вида о новых событиях связанных с пользовательским вводом
 * и передаёт данные в Модель;
 * Реагирует на уведомления от Модели о новых изменениях и передаёт данные в Вид,
 * для изменения отображения слайдера;
...
class Presenter {
  ...
  private addObserverListeners(): void {
    ...
    this.observer.subscribe('clickOnTheScale',
      (cursorPosition: ICursorPosition) => {
        this.model.moveThumbToClickPosition(cursorPosition);
      });
   ...
   * @param {boolean} isStartPosition - флаг расстановки бегунков на стартовые позиции.
   * Если true - бегунки устанавливаются на стартовые позиции.
   ...
   * Максимальный размер точки шкалы необходим для того,
   * чтобы все точки были одинаковыми и для избежания их наложения
   ...
   * @returns {boolean} - флаг видимости значений бегунков.
   * true - значение отображается, false - нет
   */
   ...
   * @param {boolean} tooltipsVisible - флаг видимости значений бегунков.
   * true - значение отображается, false - нет
   */

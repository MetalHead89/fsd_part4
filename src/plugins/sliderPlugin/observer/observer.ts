import { IObservable, IObserverFunc } from '../interfaces';
...
class Observable {
   ...
   * которая вызывается после получения наблюдателем уведомления notification
   */
  ...
  unsubscribe<T>(notification: string, func: IObserverFunc<T>): void {
    this.observables[notification] = this.observables[notification].filter(
      (subscriberfunc) => subscriberfunc !== func,
    );
  }

  /**
   * Отправляет наблюдателю уведомление notification,
   * при получении которого будет запущена привязанная к нему
  ...
  notify<T>(notification: string, data: T): void {
    if (this.observables[notification]) {
      this.observables[notification].forEach((listener: IObserverFunc<T>) => {
        listener(data);
      });
    }
  }

...
class ModelCalculator {
  ...
  /**
   * Получает объект с позицией первого бегунка,
   * приравнивает его свойства к допустимым значениям и передаёт его в метод thumbDrag,
   * который осуществляет передвижение бегунка в соответствии с полученными данными
   * @param {IThumbPosition} thumbPosition - сырая позиция бегунка,
   * может выходить за пределы слайдера или второго бегунка
   */
  dragThumbOne(thumbPosition: IThumbPosition): void {
    ...
    const position = this.getElementPosByOrientation(this.data.getThumbOnePosition());
    const thumbValue: number = this.positionToValue(position);

    this.thumbDrag(correctThumOnePosition, 'thumbOneDragged', thumbValue);
  }

  /**
   * Получает объект с позицией вторго бегунка,
   * приравнивает его свойства к допустимым значениям и передаёт его в метод thumbDrag,
   * который осуществляет передвижение бегунка в соответствии с полученными данными
   * @param {IThumbPosition} thumbPosition - сырая позиция бегунка,
   * может выходить за пределы слайдера или первого бегунка
   */
  dragThumbTwo(thumbPosition: IThumbPosition): void {
    ...
    const position = this.getElementPosByOrientation(this.data.getThumbTwoPosition());
    const thumbValue: number = this.positionToValue(position);
  ...
  /**
   * Вычисляет находится ли второй бегунок ближе к месту клика, чем первый
   * @param {IThumbPosition} position - позиция клика
   * @returns {boolean} - логическое значение с информацией о том,
   * что второй бегунок находится ближе к месту клика, чем первый
   */
  clickCloserToThumbTwo(position: IThumbPosition): boolean {
    const distanceToFirstThumb = Math.abs(this.getElementPosByOrientation(position)
      - this.getElementPosByOrientation(this.data.getThumbOnePosition()));
    const distanceToSecondThumb = Math.abs(this.getElementPosByOrientation(position)
      - this.getElementPosByOrientation(this.data.getThumbTwoPosition()));

    return this.data.getSliderType() === 'range' && distanceToFirstThumb > distanceToSecondThumb;
  }

  /**
   * Перемещает ближайший бегунок на место клика
   * @param {ICursorPosition} cursorPosition - положение курсора относительно левого
   * и правого края родительского контейнера
   */
  moveThumbToClickPosition(cursorPosition: ICursorPosition): void {
    ...
    if (this.clickCloserToThumbTwo(position)) {
  ...
  setThumbOneToStartingPosition(): void {
    ...
    } else if (this.data.getSliderType() === 'single') {
      this.dragThumbOne({
        left: 0,
  ...
  /**
   * Генерирует шкалу исходя из размеров слайдера,
   * количества шагов, минимального и максимального значения
   * */
  generateScale(): void {
    let scalePointPosition = this.getElementSizeByOrientation(this.data.getThumbSize()) / 2
    ...
    for (let i = 0; i <= Math.round(scalePointsCount - 1); i += 1) {
      const pointValue = this.positionToValue(scalePointPosition
        - this.getElementSizeByOrientation(this.data.getThumbSize()) / 2
        + this.getElementSizeByOrientation(this.data.getScalePointSize()) / 2);
        ...
        this.observer.notify('addScalePoint',
          {
            ...
            scalePointSize: this.getElementSizeByOrientation(this.data.getScalePointSize()),
      ...
      // Установка высоты и ширины шкалы для того,
      // чтобы по ней можно было совершать клики, для перемещения бегунков
   ...
   * @param {ICoordsForMargins} elements - объект с координатами слайдера,
   * шкалы и значения позиции бегунка
    ...
    if (this.data.getOrientation() === 'horizontal') {
      if (elements.sliderCoords != null) {
        if (elements.scaleCoords !== null) {
          bottomMargin = elements.scaleCoords.bottom - elements.sliderCoords.bottom;
        }
        if (elements.tooltipCoords !== null) {
          topMargin = elements.sliderCoords.top - elements.tooltipCoords.top;
        }
      }
    } else if (elements.sliderCoords !== null) {
      if (elements.tooltipCoords !== null) {
        leftMargin = elements.sliderCoords.left - elements.tooltipCoords.left;
      }
      if (elements.scaleCoords !== null) {
        rightMargin = elements.scaleCoords.right - elements.sliderCoords.right;
      }
    }
  ...
  getTooltipPosition(thumbPosition: IThumbPosition): ITooltipPosition {
    ...
    if (orientation === 'horizontal') {
      leftPos = thumbPosition.left + thumbSize.width / 2;
    } else {
      topPos = thumbPosition.top + thumbSize.height / 2;
    }
  ...
  */
   ...
   * @param {string} notyfyMessage - текст уведомления, по которому определяется бегунок,
   * который необходимо передвинуть
   * @param {number} thumbValue - значение позиции, на которой находиться бегунок
   */
  private thumbDrag(thumbPos: IThumbPosition,
    notyfyMessage: string, thumbValue: number): void {
    const newThumbPosition = { ...thumbPos };
    ...
    this.observer.notify(notyfyMessage, {
      thumbPosition: newThumbPosition,
      tooltipValue: thumbValue,
    });
   ...
   * @param {ISliderSize | IThumbSize | IScalePointSize} element - объект,
   * ширину или длину которого необходимо получить
   */
  ...
  /**
   * Возвращает положение переданного объекта,
   * относительно левого или верхнего края родительского контейнера,
   * в зависимости от ориентации слайдера.
   * При горизонтальной ориентации возвращается положение относительно
   * левого края родительского контейнера,
   ...
   * @returns {IThumbPosition} - объект с позицией бегунка.
   * Позиция считается от левого и верхнего краёв родительского контейнера
   */
  private changePositionAccordingToStep(position: IThumbPosition): IThumbPosition {
    const newPosition: IThumbPosition = { left: 0, top: 0 };

    newPosition.left = Math.round(position.left / this.calculateStepSize())
      * this.calculateStepSize();
    newPosition.top = Math.round(position.top / this.calculateStepSize())
      * this.calculateStepSize();
  ...
  private positionToValue(position: number): number {
    ...
    return Math.round(this.data.getMin() + (((this.data.getMax() - this.data.getMin()) / 100)
  ...
  private calculatePixelsPerValue(): number {
    return (this.getElementSizeByOrientation(this.data.getSliderSize())
      - this.getElementSizeByOrientation(this.data.getThumbSize())) / 100;
  }

  /**
   * Возвращает размер и точку начала прогрессбара в зависимости от текущего положения бегунков
   * @returns {IProgressBarPosition} - объект с позицией прогресс бара.
   * Содержит сведения об ориентации, точке начала и ширине (или высоте,
   * в зависимости от ориентации слайдера) прогресс бара
   */
  private calcProgressBarPosition(): IProgressBarPosition {
    ...
    if (this.data.getSliderType() === 'single') {
      progress.start = 0;
      progress.end = this.getElementPosByOrientation(this.data.getThumbOnePosition())
        + this.getElementSizeByOrientation(this.data.getThumbSize());
    } else {

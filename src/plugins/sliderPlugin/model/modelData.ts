...
class ModelData {
  ...
  setSliderSize(sliderSize: ISliderSize): void {
    const newWidth: number = (sliderSize.width >= 0) ? sliderSize.width : 0;
    const newHeight: number = (sliderSize.height >= 0) ? sliderSize.height : 0;

    this.sliderSize = { width: newWidth, height: newHeight };
  }
  ...
  setThumbSize(thumbSize: IThumbSize): void {
    const newWdth: number = (thumbSize.width >= 0) ? thumbSize.width : 0;
    const newHeight: number = (thumbSize.height >= 0) ? thumbSize.height : 0;

    this.thumbSize = { width: newWdth, height: newHeight };
  }
   ...
   * Если новое значение превышает максимальное, то оно остаётся прежним,
   * а операция считается не успешной.
   ...
   * Если новое значение меньше минимального, то оно остаётся прежним,
   * а операция считается не успешной.
   ...
   * Если одно из свойств переданного объекта больше соответствующего свойства
   * второго бегунка слайдера, то оно приравнивается к этому свойству
   ...
   * Если одно из свойств переданного объекта имеет отрицательное значение,
   * то оно приравнивается к 0
   * @param {IThumbPosition} position - объект содержащий новую позицию бегунка относительно левого
   * и верхнего края родительского контейнера
   */
  setThumbOnePosition(position: IThumbPosition): void {
    let { left: newLeft, top: newTop } = this.getThumbOnePosition();

    if (!this.thumbIsOutOfRangeHorizontally(position)) {
      newLeft = (position.left >= 0) ? position.left : 0;
    } else {
      newLeft = this.type === 'single'
        ? (this.getSliderSize().width
          - this.getThumbSize().width) : this.getThumbTwoPosition().left;
    }

    if (!this.thumbIsOutOfRangeVertically(position)) {
      newTop = (position.top >= 0) ? position.top : 0;
    } else {
      newTop = this.type === 'single'
        ? (this.getSliderSize().height
          - this.getThumbSize().height) : this.getThumbTwoPosition().top;
    }

    this.thumbOnePosition = { left: newLeft, top: newTop };
  }
   ...
   * @param {IThumbPosition} position - объект содержащий новую позицию бегунка относительно
   ...
   * @returns {IThumbPosition} - объект содержащий позицию бегунка относительно левого
   * и верхнего края родительского контейнера
   */
  ...
  /**
   * Устанавливает позицию бегунка относительно левого и верхнего края родительского контейнера.
   * Если одно из свойств переданного объекта меньше соответствующего свойства первого бегунка,
   * то оно приравнивается к этому свойству.
   * Если одно из свойств переданного объекта превышает максимально возможную позицию бегунка,
   * ограниченную размерами слайдера,
  ...
  setThumbTwoPosition(position: IThumbPosition): void {
    ...
    if (position.left >= this.getThumbOnePosition().left) {
      newLeft = (position.left <= (this.getSliderSize().width - this.getThumbSize().width))
        ? position.left : (this.getSliderSize().width - this.getThumbSize().width);
    } else {
      newLeft = this.getThumbOnePosition().left;
    }
    if (position.top >= this.getThumbOnePosition().top) {
      newTop = (position.top <= (this.getSliderSize().height - this.getThumbSize().height))
        ? position.top : (this.getSliderSize().height - this.getThumbSize().height);
    } else {
      newTop = this.getThumbOnePosition().top;
    }

    this.thumbTwoPosition = { left: newLeft, top: newTop };
  }

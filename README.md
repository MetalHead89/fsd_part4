...
Simple slider plugin - простой в использовании слайдер для быстрого выбора значения или диапазона значений.
...
3. Произвести импорт плагина в точке входа проекта:
    ...
    import 'путьДоКаталогаSlider/slider.js'

4. Произвести импорт стилей плагина в точке входа проекта:

    import 'путьДоКаталогаSlider/styles.css'
...
Вторым шагом будет вызов функции sliderPlugin() у обёрнутого в jQuery элемента, созданного ранее:

	$(sliderWrapper).sliderPlugin();
...
Настройки по-умолчанию удобны для быстрого создания нового слайдера, но в подавляющем большинстве случаев, необходим слайдер с иным диапазоном значений или другим типом. Для этого в плагине предусмотрена возможность конфигурирования настроек при создании слайдера. Всё что требуется сделать - это передать в метод sliderPlugin() объект с необходимыми настройками, специфичными для текущего проекта:

	$(sliderWrapper).sliderPlugin({'min': 60, 'max': 4000, 'orientation': 'vertical'});
...
Помимо конфигурирования во время создания, плагином предусмотрен API для динамической смены настроек слайдера, уже размещённого в DOM. Для обращения к API нужно передать в функцию sliderPlugin() два аргумента:

	$(sliderWrapper).sliderPlugin(functionAPI, arg);
...
Например, для того, чтобы получить текущие значения бегунков, необходимо выполнить следующую команду:
	...
	$(sliderWrapper).sliderPlugin(getThumbsValues);

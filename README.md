...
>Внимание! Для работы плагина, проект должен поддерживать работу с TypeScript, scss и jQuery. Если во время работы со слайдером возникают ошибки, необходимо убедиться, что выполнена установка соответствующих пакетов и произведена настройка сборщика для работы с ними.

Для добавления плагина в проект необходимо:

1. Скопировать каталог sliderPlugin из репозитория проекта:

    https://github.com/MetalHead89/fsd_part4/tree/master/src/plugins

2. Произвести импорт плагина в точке входа проекта:

    import 'путьДоКаталогаSliderPlugin/sliderPlugin/incredibleSliderPlugin.ts'

3. Произвести импорт стилей плагина в точке входа проекта:
...
Настройки по-умолчанию удобны для быстрого создания нового слайдера, но в подавляющем большинстве случаев, необходим слайдер с иным диапазоном значений или другим типом. Для этого в плагине предусмотрена возможность конфигурирования настроек при создании слайдера. Всё что требуется сделать - это передать в метод incredibleSliderPlugin() объект с необходимыми настройками, специфичными для текущего проекта:
...
Например, для того, чтобы получить текущие значения бегунков, необходимо выполнить следующую команду:
	...
	$(sliderWrapper).incredibleSliderPlugin(getThumbsValues);
